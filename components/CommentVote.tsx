"use client"
import { toggleCommentVote } from '@/app/action';
...
const CommentVote = ({ commentId, initialVotes, initialHasVoted }: CommentVoteProps) => {
  ...
  return (
    <button
      onClick={handleVote}
      className={`flex items-center gap-1.5 text-sm font-medium transition-colors ${optimisticState.hasVoted
        ? 'text-amber-600 bg-amber-50 px-2 py-1 rounded-md'
        : 'text-neutral-500 hover:text-amber-600 hover:bg-neutral-50 px-2 py-1 rounded-md'
        }`}
    >
      <svg
        width="24"
        ...
        viewBox="0 0 24 24"
        fill="currentColor"
        className={`w-5 h-5 ${optimisticState.hasVoted ? 'text-amber-600' : 'text-neutral-400'}`}
